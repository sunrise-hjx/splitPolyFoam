...
namespace Foam
{
defineTypeNameAndDebug(MYcellSplitter, 0);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::MYcellSplitter::getFaceInfo
(
    const label facei,
    label& patchID,
    label& zoneID,
...
// newCells
Foam::label Foam::MYcellSplitter::newOwner
(
    const label facei,
...
Foam::label Foam::MYcellSplitter::newNeighbour
(
    const label facei,
    const Map<labelList>& cellToCells
) const
...
Foam::MYcellSplitter::MYcellSplitter(const polyMesh& mesh)
:
    mesh_(mesh),
    addedPoints_()
{}
...
void Foam::MYcellSplitter::setRefinement
(
...
void Foam::MYcellSplitter::updateMesh(const mapPolyMesh& morphMap)
{
